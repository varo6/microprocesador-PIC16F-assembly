    ...
    ; CICLO_100us every 100 �s. CICLO_100us must be cleared at initialization.
    LIST P= 16f876A
    ...
    ORG 4                ; Definimos la direcci�n de la interrupci�n.
    goto	int_tmr
    ORG	5
    ; Importante: No es lo mismo incrementar 1 vez cada ciclo que 1 vez cada 100us. Cuando tmr0 desborde ser� cuando sumaremos 1.
inicio

    BSF 	INTCON,GIE      ; Habilitar la interrupci�n
    ...
    BSF		OPTION_REG,PSA  ; Ponemos prescaler a 1 para que sea de watchdog timer
                         ; Si queremos el rate1:1 PSA tendr� que ser del WDT(a 1), para otros rate tenemos que poner prescaler a 0 para que sea de timer0
...
int_tmr
    ...
    movlw	.156           ; Como TMR0 se desborda inicia de 0, por lo que volvemos a sumarle 156.
    movwf	TMR0           ; pasa de wreg a TMR0
retfie
END
